...
function CompanySettings() {
  ...
  const [formData, setFormData] = useState<any>({
    ...
    booksStartDate: '', // Moved to top-level
    fiscalYearStartDate: '', // Moved to top-level
    fiscalYearEndDate: '', // Auto-calculated, not directly editable
    defaultCurrency: '', // Moved to top-level
    decimalPlaces: 2, // Moved to top-level
  ...
  useEffect(() => {
    if (currentCompany) {
      ...
      setFormData({
        ...
        bankDetails: currentCompany.settings?.bankDetails || { bankName: '', accountNumber: '', ifscCode: '', swiftCode: '', accountHolderName: '', currency: '' }, // NEW
      });
    }
  }, [currentCompany]);
  ...
  const validateForm = (currentTabId: string): Record<string, string> => {
    ...
    if (currentTabId === 'bank_details') {
      if (formData.bankDetails.bankName || formData.bankDetails.accountNumber || formData.bankDetails.ifscCode || formData.bankDetails.swiftCode || formData.bankDetails.accountHolderName || formData.bankDetails.currency) {
        ...
        if (!formData.bankDetails.accountHolderName.trim()) newErrors.accountHolderName = 'Account Holder Name is required if bank details are provided'; // NEW
        if (!formData.bankDetails.currency.trim()) newErrors.currency = 'Currency is required if bank details are provided'; // NEW
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const selectedPhoneCountryDataForSubmission = getPhoneCountryCodes().find(c => c.code === formData.phoneCountry);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex items-center space-x-3">
                    <input
                      ...
                      checked={formData.multiUserAccess}
