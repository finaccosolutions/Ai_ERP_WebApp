...
import { COUNTRIES, getCountryByCode, getPhoneCountryCodes, globalBusinessTypes } from '../../constants/geoData'; // Import geoData
...
interface Company {
  ...
  taxConfig: {             // Changed from tax_config
    type: 'GST' | 'VAT' | 'Sales Tax' | 'GST/PST/HST' | 'Consumption Tax' | 'Custom';
    rates: number[];
    enabled: boolean;
...
const getInitialFormData = (company?: Company) => {
  ...
  const defaultCountry = COUNTRIES.find(c => c.code === (company?.country || 'IN'))!;
  ...
  const defaultContactInfo = { contactPersonName: '', designation: '', email: '', mobile: '', alternatePhone: '', phoneCountry: defaultCountry.dialCode };
  const defaultTaxConfig = {
    enabled: true,
    rates: defaultCountry.taxConfig.rates,
  ...
  const defaultSettings = {
    displayName: '', legalName: '', industry: industries[0].id, businessType: globalBusinessTypes[0].id, // Use globalBusinessTypes as default
    registrationNo: '',
    languagePreference: languages[0].id, decimalPlaces: defaultCountry.defaultDecimalPlaces, multiCurrencySupport: false, autoRounding: false,
    ...
    barcodeSupport: false, autoVoucherCreationAI: false, companyType: globalBusinessTypes[0].id, // Use globalBusinessTypes as default
    employeeCount: employeeCounts[0].id,
    annualRevenue: revenueRanges[0].id, inventoryTracking: true, companyUsername: ''
  };
  ...
  return {
    ...
    businessType: safeCompany.settings.businessType || defaultSettings.businessType, // Use defaultSettings
    ...
    phoneCountry: safeCompany.contactInfo.phoneCountry || defaultContactInfo.phoneCountry,
    alternateContactNumber: safeCompany.contactInfo.alternatePhone || '',

    taxSystem: safeCompany.taxConfig.type || defaultTaxConfig.type,
    taxConfig: {
    ...
    gstRegistrationType: safeCompany.taxConfig.gstDetails?.registrationType || gstRegistrationTypes[0].id,
    filingFrequency: safeCompany.taxConfig.gstDetails?.filingFrequency || filingFrequencies[0].id,
    ...
    vatRegistrationType: safeCompany.taxConfig.vatDetails?.registrationType || vatRegistrationTypes[0].id,
    filingCycle: safeCompany.taxConfig.vatDetails?.filingCycle || filingCycles[0].id,
    ...
    enableBatchTracking: safeCompany.settings.batchTracking ?? defaultSettings.enableBatchTracking,
    costCenterAllocation: safeCompany.settings.costCenterAllocation ?? defaultSettings.costCenterAllocation,
    enableMultiUserAccess: safeCompany.settings.multiUserAccess ?? defaultSettings.enableMultiUserAccess,
    allowAiSuggestions: safeCompany.settings.aiSuggestions ?? defaultSettings.allowAiSuggestions,
    companyPassword: safeCompany.settings.password || '',
    enableCompanyPassword: safeCompany.settings.enablePassword ?? defaultSettings.enableCompanyPassword,
    allowSplitByPeriod: safeCompany.settings.splitByPeriod ?? defaultSettings.allowSplitByPeriod,
    enableBarcodeSupport: safeCompany.settings.barcodeSupport ?? defaultSettings.enableBarcodeSupport,
    allowAutoVoucherCreationAI: safeCompany.settings.autoVoucherCreationAI ?? defaultSettings.allowAutoVoucherCreationAI,

    companyType: safeCompany.settings.companyType || defaultSettings.companyType,
    employeeCount: safeCompany.settings.employeeCount || defaultSettings.employeeCount,
    annualRevenue: safeCompany.settings.annualRevenue || defaultSettings.annualRevenue,
...
function CompanySetup({ companyToEdit, readOnly, onSaveSuccess, onSaveError, onCancel }: CompanySetupProps) {
    ...
    if (!companyToEdit || !formData.fiscalYearStartDate || !formData.fiscalYearEndDate) { // Added companyToEdit check
      ...
      if (selectedCountryData) {
        ...
        setFormData((prev: any) => ({
          ...
          phoneCountry: selectedCountryData.dialCode, // Auto-update phone country code
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setFormData((prev: any) => ({ ...prev, companyLogo: URL.createObjectURL(file) })); // For preview
      // Store the actual file object for upload
      // This part needs to be handled carefully if you're using a separate state for the file object
      // For simplicity, I'm assuming `logoFile` state is used for the actual upload.
      // You might need to adjust this based on your actual file upload mechanism.
    }
  };
...
const handleSubmit = async () => {
    ...
    try {
      ...
      if (companyToEdit) {
        ...
        if (companyId) {
          ...
          await populateDefaultChartOfAccounts(supabase, companyId, formData.country); // Pass country code
  ...
  const selectedPhoneCountry = COUNTRIES.find(c => c.dialCode === formData.phoneCountry); // Use dialCode for phoneCountry
  const availableStates = selectedCountry?.states.map(s => s.name) || []; // Map to names
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className={theme.textSecondary}>
                {readOnly ? 'Review company information.' : 'Update your company information.'}
              </p>
            </div>
            {onCancel && (
              <Button
                  ...
                  <div className="space-y-2"> {/* NEW: Alternate Contact Number with country code */}
                    ...
                    <div className="flex items-stretch">
                      <div className="relative">
                        ...
                        {showAlternatePhoneCountryDropdown && !readOnly && (
                          <div className={`
                            absolute top-full left-0 mt-1 w-64 ${theme.isDark ? 'bg-gray-700' : 'bg-white'}
                            border ${theme.isDark ? 'border-gray-600' : 'border-gray-300'}
                            ${theme.borderRadius} shadow-lg z-50 max-h-60 overflow-y-auto
                          `}>
                            {getPhoneCountryCodes().map((country) => (
                              <button
                                ...
                                type="button"
                                onClick={() => {
                                  setFormData({ ...formData, phoneCountry: country.id }); // Assuming same phoneCountry for both
