// src/pages/Inventory/masters/ItemCategoriesGroupsPage.tsx
...
function ItemCategoriesGroupsPage() {
  ...
  const [numCategoriesToShow, setNumCategoriesToShow] = useState<string>('10'); // Default to 10

  useEffect(() => {
    if (currentCompany?.id) {
      fetchCategories();
    }
  }, [currentCompany?.id, filterCriteria, numCategoriesToShow]); // Added filterCriteria and numCategoriesToShow to dependencies
  ...
  const fetchCategories = async () => {
    ...
    try {
      let query = supabase
        ...
        .select(`
          *,
          parent_category:item_categories ( name )
        `, { count: 'exact' })
      ...
      showNotification(`Error fetching item categories: ${err.message}`, 'error');
  ...
  const confirmDeleteCategory = async () => {
    ...
    try {
      ...
      if (linkedItemsCount && linkedItemsCount > 0) {
        showNotification(`Cannot delete category: ${linkedItemsCount} item(s) are linked to it. Please reassign them first.`, 'error');
        setLoading(false);
        return;
      }
      ...
      if (error) throw error;
      showNotification('Item category deleted successfully!', 'success');
      fetchCategories();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className={`text-3xl font-bold ${theme.textPrimary}`}>Item Categories / Groups</h1>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="flex items-center justify-center h-48 border border-dashed rounded-lg text-gray-500">
              <p>No item categories or groups found. Create a new one to get started.</p>
            </div>
          ) : (
...
export default ItemCategoriesGroupsPage;
