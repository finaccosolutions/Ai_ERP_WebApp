// src/pages/Inventory/transactions/StockTransfersPage.tsx
import React, { useState, useEffect } from 'react';
import { Plus, Search, ArrowLeftRight, Edit, Trash2, RefreshCw, Save, ArrowLeft, Filter } from 'lucide-react';
...
import StockTransferFilterModal from '../../../components/Modals/StockTransferFilterModal';
...
function StockTransfersPage() {
  ...
  const [stockTransfers, setStockTransfers] = useState<StockEntry[]>([]);
  ...
  const fetchStockTransfers = async () => {
    ...
    try {
      let query = supabase
        ...
        .eq('entry_type', 'material_transfer') // Filter for stock transfers
      ...
      if (filterCriteria.fromWarehouseId) {
        query = query.eq('from_warehouse_id', filterCriteria.fromWarehouseId);
      }
      if (filterCriteria.toWarehouseId) {
        query = query.eq('to_warehouse_id', filterCriteria.toWarehouseId);
      }
      ...
      showNotification(`Error fetching stock transfers: ${err.message}`, 'error');
  ...
  const validateForm = () => {
    ...
    if (!formData.fromWarehouseId) {
      showNotification('From Warehouse is required.', 'error');
      return false;
    }
    if (!formData.toWarehouseId) {
      showNotification('To Warehouse is required.', 'error');
      return false;
    }
    if (formData.fromWarehouseId === formData.toWarehouseId) {
      showNotification('From and To Warehouses cannot be the same.', 'error');
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const stockEntryToSave = {
        ...
        entry_type: 'material_transfer', // Fixed for this page
      ...
      if (formData.id) {
        const { error } = await supabase
          ...
          .eq('id', formData.id);
        if (error) throw error;
        showNotification('Stock transfer updated successfully!', 'success');
      } else {
  ...
  const handleEditEntry = async (entry: StockEntry) => {
    ...
    try {
      ...
      setFormData({
        ...
        fromWarehouseId: entry.from_warehouse_id || '',
        toWarehouseId: entry.to_warehouse_id || '',
        notes: entry.notes || '',
        status: entry.status,
      ...
      showNotification(`Error loading stock transfer for edit: ${err.message}`, 'error');
      console.error('Error loading stock transfer for edit:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDeleteEntry = async () => {
    ...
    setLoading(true);
    try {
      await supabase.from('stock_entry_items').delete().eq('entry_id', transferToDeleteId);
      const { error } = await supabase
        .from('stock_entries')
        .delete()
        .eq('id', transferToDeleteId);

      if (error) throw error;
      showNotification('Stock transfer deleted successfully!', 'success');
      fetchStockTransfers();
    } catch (err: any) {
      showNotification(`Error deleting stock transfer: ${err.message}`, 'error');
  ...
  const numResultsOptions = [
    ...
    { id: '25', name: 'Show 25' },
    { id: '50', name: 'Show 50' },
    { id: 'all', name: `Show All (${totalTransfersCount})` },
  ];
    ...
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className={`text-3xl font-bold ${theme.textPrimary}`}>
              {viewMode === 'create' ? 'Record Stock Transfer' : 'Edit Stock Transfer'}
            </h1>
            <p className={theme.textSecondary}>
              {viewMode === 'create' ? 'Transfer stock between warehouses.' : 'Update stock transfer details.'}
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card className="p-6">
            <h3 className={`text-lg font-semibold ${theme.textPrimary} mb-4`}>Transfer Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <MasterSelectField
                ...
                value={availableWarehouses.find(wh => wh.id === formData.fromWarehouseId)?.name || ''}
                onValueChange={(val) => {}}
                onSelect={(id) => handleInputChange('fromWarehouseId', id)}
              ...
              <MasterSelectField
                label="To Warehouse"
                value={availableWarehouses.find(wh => wh.id === formData.toWarehouseId)?.name || ''}
                onValueChange={(val) => {}}
                onSelect={(id) => handleInputChange('toWarehouseId', id)}
                options={availableWarehouses}
          ...
          <Card className="p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className={`text-lg font-semibold ${theme.textPrimary}`}>Items to Transfer</h3>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className={`text-3xl font-bold ${theme.textPrimary}`}>Stock Transfers</h1>
          <p className={theme.textSecondary}>Move inventory between different warehouses or locations.</p>
        </div>
        <div className="flex space-x-2">
          <Button variant="outline" onClick={() => navigate('/inventory')} icon={<ArrowLeft size={16} />}>
            Back
          </Button>
          <AIButton variant="suggest" onSuggest={() => console.log('AI Transfer Suggestions')} />
      ...
      <Card className="p-6">
        <h3 className={`text-lg font-semibold ${theme.textPrimary} mb-4`}>All Stock Transfers</h3>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0 sm:space-x-4 mb-6">
          <div className="relative flex-grow">
            ...
            <input
              ...
              onKeyPress={(e) => e.key === 'Enter' && fetchStockTransfers()}
              className={`
          ...
          <MasterSelectField
            ...
            className="w-32"
          />
          <Button onClick={fetchStockTransfers} disabled={loading} icon={<RefreshCw size={16} />}>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="flex items-center justify-center h-48 border border-dashed rounded-lg text-gray-500">
              <p>No stock transfers found. Record a new one to get started.</p>
            </div>
          ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {stockTransfers.map((entry) => (
                  <tr key={entry.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{entry.to_warehouses?.name || 'N/A'}</td>
      ...
      <ConfirmationModal
        ...
        title="Confirm Stock Transfer Deletion"
        message="Are you sure you want to delete this stock transfer entry? This action cannot be undone."
